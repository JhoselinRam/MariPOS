...
function NewSuplier({list}:newSuplierProps){
    ...
    const [action, setAction] = useState(`${process.env.REACT_APP_ACTIONS_NEW_SUPLIER}`);
    ...
    function submitSuplier(){
        let passwordButton = document.getElementById("submitPasswordToggler") as HTMLButtonElement;
    ...
    function editSuccessful(response:SubmitPasswordResponse){
    ...
    return (
        <>
            ...
            <SubmitPassword action={action} onSuccess={
                    action===`${process.env.REACT_APP_ACTIONS_NEW_SUPLIER}`?submitSuccessful : editSuccessful
                } onFailure={submitFailure} onClose={paswordClose} parent="#NewSuplierPanel"/>
        </>        
    );
}
